...
describe("UpgradeTemplatesDialogComponent", () => {
  ...
  describe("locking the dialog while a run is created", () => {
    async function openExecutor() {
      ...
      await user.click(screen.getByRole("button", { name: "Run Continuous RTP Greening" }));
      await waitFor(() =>
        expect(ngMocks.findInstance(UpgradeExecutorComponent)).toBeTruthy()
      );
      return view;
    ...
    it("blocks navigating back while the run is being created", async () => {
      ...
      ngMocks.findInstance(UpgradeExecutorComponent).executingChange.emit(true);
      detectChanges();
    ...
    it("lets the user navigate back again once the run finishes", async () => {
      ...
      ngMocks.findInstance(UpgradeExecutorComponent).executingChange.emit(false);
      detectChanges();
  ...
  describe("cancelling the executor", () => {
    it("returns to the template list when the executor is cancelled", async () => {
      ...
      ngMocks.findInstance(UpgradeExecutorComponent).cancelled.emit();
      detectChanges();
