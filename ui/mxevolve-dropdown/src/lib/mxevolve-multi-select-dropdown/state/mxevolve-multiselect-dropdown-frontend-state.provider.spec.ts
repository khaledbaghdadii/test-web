...
import { MxevolveMultiselectFrontendStateProvider } from "./mxevolve-multiselect-dropdown-frontend-state.provider";
...
describe("MxevolveMultiselectFrontendStateProvider", () => {
  let service: MxevolveMultiselectFrontendStateProvider<
    TestItem,
    { testParam: string }
  >;
  ...
  beforeEach(() => {
    ...
    service = new MxevolveMultiselectFrontendStateProvider(
      mockDataProvider,
      mockDestroyRef,
      200
    );
  ...
  describe("dropdownOptions", () => {
    ...
    it("should include selected items not in filtered list at the top", fakeAsync(() => {
      service.setDataParams({ testParam: "test" });
      tick(250);
      ...
      // Filter to only show items containing 'ban' (Banana)
    ...
    it("should not duplicate selected items if already in filtered list", fakeAsync(() => {
      service.setDataParams({ testParam: "test" });
      tick(250);
  ...
  describe("setPageIndex", () => {
    it("should be a no-op for frontend filtering", () => {
      expect(() => service.setPageIndex()).not.toThrow();
    });
  });
