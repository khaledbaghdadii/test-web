...
type SendForReviewStateServiceMock = {
  setProjectId: jest.Mock;
  setRepositoryId: jest.Mock;
  setPageIndex: jest.Mock;
  setFilterReset: jest.Mock;
  setFilter: jest.Mock;
  errorMessage: ReturnType<typeof signal<string | undefined>>;
  reviewerSuggestions: ReturnType<typeof signal<unknown[] | undefined>>;
  isLastPage: ReturnType<typeof signal<boolean>>;
  isLoadingData: ReturnType<typeof signal<boolean>>;
  pageIndex: ReturnType<typeof signal<number>>;
};

describe("Reviewers AutoComplete Component", () => {
  let sendForReviewStateService: SendForReviewStateServiceMock;
  ...
  function initMocks() {
    ...
    } as unknown as jest.Mocked<DevelopmentService>;
  }
  ...
  describe("Getting default reviewers", () => {
    ...
    it("should show the default reviewers warning message if an error occurred while fetching them", fakeAsync(() => {
      jest
        .spyOn(reviewersService, "getDefaultReviewers")
    ...
    it("should hide the default reviewers warning message when fetching them again if an error occurred while fetching before", fakeAsync(() => {
      ...
      fixture.detectChanges();
      expect(component.showReviewersWarningMessage).toBeTruthy();
      jest.spyOn(reviewersService, "getDefaultReviewers").mockReturnValue(
        of({
          content: [REVIEWER],
        })
      );
    ...
    it("should set repository id on state service when development is resolved", fakeAsync(() => {
      component.ngOnInit();
      tick();
      fixture.detectChanges();
