...
import { ValidationMqgParametersComponent } from "./validation-mqg-parameters.component";
...
const MOCK_IMPORTS = [
  ReactiveFormsModule,
  RadioButton,
  MockComponent(MqgFromNewBranchParametersComponent),
...
async function renderComponent({
  ...
  const view = await render(ValidationMqgParametersComponent, {
    inputs: {
      projectId: "project-1",
      providedInputs,
      repositoryId: "repo-1",
...
describe("ValidationMqgParametersComponent", () => {
  /**
   * Legacy wrapped the radio group in `mxevolve-definition-input` with
   * `ACCESS_INVALID_INPUTS_ONLY`, so a definition that already answered
   * "Create Branch?" did not ask again. The migrated component rendered a bare
   * fieldset with no gate at all.
   */
  ...
  describe("sub-form selection", () => {
    it("shows the new-branch parameters when the answer is Yes", async () => {
      ...
      expect(
        document.querySelector("mxevolve-mqg-from-new-branch-parameters")
      ).toBeTruthy();
      expect(
