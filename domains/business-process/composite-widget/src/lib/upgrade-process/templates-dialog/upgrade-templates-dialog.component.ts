...
} from "../executor/upgrade-executor.form";

/**
 * Upgrade "Available Templates" dialog — Page 1 of the generic multi-page
 * dialog (VAL-27132 Step 18). Loads the project's business-process definitions
 * once, UI-filters them to the Upgrade family, lets the user narrow by a
 * dynamically-derived Sub-Activity, and starts a run per row.
 *
 * Page 2 (the per-family executor) is the upgrade Reactive-Forms executor
 * (Step 19), projected into the same multi-page dialog; this component declares
 ...
 */
@Component({
  selector: "mxevolve-upgrade-templates-dialog",
  templateUrl: "./upgrade-templates-dialog.component.html",
  styleUrl: "./upgrade-templates-dialog.component.scss",
  imports: [
    MultiPageDialogComponent,
    MultiPageDialogPageDirective,
    ...
    Button,
    MxevolveIconComponent,
    UpgradeExecutorComponent,
    DefinitionDetailsLinkComponent,
  ],
  providers: [BusinessProcessDefinitionService, ExecutionFetcherService],
})
export class UpgradeTemplatesDialogComponent {
  ...
  protected readonly executorSeed = signal<UpgradeExecutorSeed | undefined>(
    undefined
  );
  ...
  private readonly upgradeSourceDefinitions = computed(() =>
    (this.sourceDefinitionsResource.value() ?? []).filter(
      (definition) => definition.family?.id === ExecutionFamily.UPGRADE_PROCESS
    )
  );

  private readonly upgradeDefinitions = computed(() =>
  ...
  protected readonly templates = computed(() => {
    ...
    return this.upgradeDefinitions().filter(
      (definition) =>
        (!subFamily ||
  ...
  protected readonly subFamilyOptions = computed<SubFamilyOption[]>(() => [
    UpgradeTemplatesDialogComponent.ALL_OPTION,
    ...deriveSubFamilies(this.upgradeSourceDefinitions()),
  ]);
  protected readonly pageSize = UpgradeTemplatesDialogComponent.PAGE_SIZE;
    ...
    this.executionFetcher
      ...
      .subscribe({
        next: ({ definition, execution }) => {
          this.executorSeed.set(toUpgradeExecutorSeed(execution));
