...
export interface UpgradeRunNameCellParams extends ICellRendererParams {
  projectId: string;
}

/**
 * Renders the run **Execution Name** as a router link to the Upgrade execution
 * view (`upgrade-activity/execution/{id}`), matching the legacy upgrade table's
 * linked execution name. Falls back to plain text when the run has no id.
 */
@Component({
  selector: "mxevolve-upgrade-run-name-cell",
  imports: [RouterLink],
...
export class UpgradeRunNameCellComponent implements ICellRendererAngularComp {
  protected name = "";
  protected processId = "";
  protected projectId = "";

  agInit(params: UpgradeRunNameCellParams): void {
    this.update(params);
  }

  refresh(params: UpgradeRunNameCellParams): boolean {
    this.update(params);
    return true;
  }

  private update(params: UpgradeRunNameCellParams): void {
...
@Component({
  selector: "mxevolve-upgrade-run-status-cell",
...
export class UpgradeRunStatusCellComponent implements ICellRendererAngularComp {
